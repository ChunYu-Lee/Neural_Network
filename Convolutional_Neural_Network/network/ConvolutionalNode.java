...
public class ConvolutionalNode {
    ...
    //the size of the Z dimension of this feature map, include padding
    public final int sizeZ;

    //the size of the Y dimension of this feature map, include padding
    public final int sizeY;

    //the size of the X dimension of this feature map, include padding
    ...
    public void propagateForward(boolean training) {
        ...
        if (nodeType == NodeType.HIDDEN) {
            ...
            if (useBatchNormalization) {
                ...
                if (training) {
                    ...
                    //mean
                    //You must zero down the mu_b before you start a new forward pass.
                    mu_b = 0.0;
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                    ...
                    //variance
                    //You must zero down the sigma2_b before you start a new forward pass.
                    sigma2_b = 0.0;
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {
                                    double difference =  inputValues[i][z][y][x] - mu_b;
                                    sigma2_b += difference * difference;
                                    }
                                }
                            }
                        }
                    ...
                    //sigma2
                    sigma2_b = sigma2_b / (number); //-1

                    //x-hat and afterBatchNorm
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {

                                    double d =  inputValues[i][z][y][x] - mu_b;
                                    xHat[i][z][y][x] = d / Math.sqrt(sigma2_b + epsilon);
                    ...
                    //update the mu_running and sigma2_running
                    mu_running = (mu_b * alpha) + ((1 - alpha) * mu_running);
                    sigma2_running = (sigma2_b * alpha) + ((1 - alpha) * sigma2_running);
                        ...
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {
                                    ...
                                    afterBatchNorm[i][z][y][x] = (gamma * inputValues[i][z][y][x] / Math.sqrt(sigma2_running + epsilon)) + (beta - gamma * mu_running / Math.sqrt(sigma2_running + epsilon));
                                    }
                                }
                            }
                        }
            ...
            if (activationType == ActivationType.NONE) {
                    ...
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                ...
                                if (inputs[i][z][y][x] + bias[z][y][x] <=0){
                                    outputValues[i][z][y][x] = 0;
                                }else{
                                    outputValues[i][z][y][x] = inputs[i][z][y][x] + bias[z][y][x];
                                    }
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                //x < 0, output = 0, 0< x <5 ,output = x, x >5, output = 5
                                if (inputs[i][z][y][x] + bias[z][y][x] <=0){
                                    outputValues[i][z][y][x] = 0;
                                }else if (inputs[i][z][y][x] + bias[z][y][x] >=5){
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                //x < 0, output = x*leak, x > 0 ,output = x
                                if (inputs[i][z][y][x] + bias[z][y][x] <=0){
                                    outputValues[i][z][y][x] = leak * (inputs[i][z][y][x] + bias[z][y][x]);
                                }else if (inputs[i][z][y][x] + bias[z][y][x] >=5){
                                    outputValues[i][z][y][x] = 5;
                                }else{
                                    outputValues[i][z][y][x] = inputs[i][z][y][x] + bias[z][y][x];
            ...
            for (int i = 0; i < batchSize; i++) {
                for (int z = 0; z < sizeZ; z++) {
                    for (int y = 0; y < sizeY; y++) {
                        for (int x = 0; x < sizeX; x++) {
                            outputValues[i][z][y][x] = inputValues[i][z][y][x];
                            }
                        }
                    }
                }
        ...
        if (nodeType == NodeType.HIDDEN || nodeType == NodeType.INPUT) {
            ...
            if (useDropout) {
                //TODO: Implement this for Programming Assignment 3 - Part 3
                //NOTE: please use generator.nextDouble() to determine if a
                ...
                if (training) {
                    ...
                    //build up the dropout map
                    for (int i = 0; i < batchSize; i++) {
                        for (int z = 0; z < sizeZ; z++) {
                            for (int y = 0; y < sizeY; y++) {
                                for (int x = 0; x < sizeX; x++) {

                                    dropoutDelta[i][z][y][x] = generator.nextDouble();

                                    if (dropoutDelta[i][z][y][x] < 1 - dropoutRate){
                                        dropoutDelta[i][z][y][x] = 1 /(1 - dropoutRate) ;
                                    }else{
                                        dropoutDelta[i][z][y][x] = 0;
                                        }

                                    //do the dropout
                                    outputValues[i][z][y][x] *= dropoutDelta[i][z][y][x];
                                    }
                                }
                            }
                        }
    ...
    public void propagateBackward() {
        ...
        if (nodeType == NodeType.HIDDEN) {
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                //x < 0, output = 0, 0< x <5 ,output = x, x >5, output = 5
                                if (inputs[i][z][y][x] + bias[z][y][x] <= 0 || inputs[i][z][y][x] + bias[z][y][x] >= 5){
                                    delta[i][z][y][x] *= 0;
                                }else{
                                    delta[i][z][y][x] *= 1;
                                    }
            ...
            if (useBatchNormalization) {
                ...
                int number = 0;
                double[][][][] muDelta = new double[batchSize][sizeZ][sizeY][sizeX];
                double[][][][] deltaGamma = new double[batchSize][sizeZ][sizeY][sizeX];
                betaDelta = 0.0;
                gammaDelta = 0.0;

                //sum  gammaDelta*(inputValues - mu_b), update betaDelta
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                //update betaDelta and gammaDelta
                                betaDelta += this.delta[i][z][y][x];
                                gammaDelta += this.delta[i][z][y][x] * xHat[i][z][y][x];
                                deltaGamma[i][z][y][x] = this.delta[i][z][y][x] * gamma;
                                sigma2_delta += deltaGamma[i][z][y][x] * (inputValues[i][z][y][x] - mu_b);
                                number ++;
                                }
                            }
                        }
                ...
                sigma2_delta = sigma2_delta / (number * Math.sqrt(Math.pow((sigma2_b + epsilon),3)));
                //sum muDelta
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                muDelta[i][z][y][x] = (deltaGamma[i][z][y][x] / Math.sqrt(sigma2_b + epsilon)) - ((inputValues[i][z][y][x] - mu_b)*sigma2_delta);
                                mu_delta += muDelta[i][z][y][x];
                                }
                            }
                        }
                    }
                ...
                for (int i = 0; i < batchSize; i++) {
                    for (int z = 0; z < sizeZ; z++) {
                        for (int y = 0; y < sizeY; y++) {
                            for (int x = 0; x < sizeX; x++) {
                                delta[i][z][y][x] = (muDelta[i][z][y][x] - mu_delta/number);
                                }
                            }
                        }
                    }
    ...
    public void initializeWeightsAndBiasKaiming(double initialBias, int fanIn) {
        ...
        for (Edge edge: inputEdges){
            double[] w = new double [edge.sizeZ * edge.sizeY * edge.sizeX];

            for (int i = 0; i < w.length; i++) {
                w[i] = rand.nextGaussian() / Math.sqrt(fanIn);
                }
    ...
    public void initializeWeightsAndBiasXavier(double initialBias, int fanIn, int fanOut) {
        ...
        for (Edge edge: inputEdges){
            ...
            for (int i = 0; i < w.length; i++) {
                w[i] = (2 * rand.nextDouble() - 1) * (Math.sqrt(6) / Math.sqrt(fanIn + fanOut));
                }
