...
public class PA13GradientDescent {
    ...
    public static void stochasticEpoch(DataSet dataSet, NeuralNetwork nn, double learningRate) throws NeuralNetworkException {
        ...
        for(int i=0; i < dataSet.getNumberInstances(); i++){
            ...
            double gradient[] = nn.getGradient(dataSet.getInstance(i));
            double temp_weights[] = nn.getWeights();
            for(int j=0; j < temp_weights.length; j++){
                temp_weights[j] -= learningRate * gradient[j];
                }
    ...
    public static void minibatchEpoch(DataSet dataSet, NeuralNetwork nn, int batchSize, double learningRate) throws NeuralNetworkException {
        ...
        //get the Random instances from the dataset
        for(int i=0; i < NumberInstances ; i += batchSize){
            //calculate all the batches till the last one
            double temp_weights[] = nn.getWeights();

            if (i < NumberInstances - batchSize){

                List<Instance> instances = dataSet.getInstances(i, batchSize);
                double gradient[] = nn.getGradient(instances);

                for(int j=0; j < temp_weights.length; j++){
                    //weight -= learningRate * gradient(that is the same of gradient of all instances)
                    temp_weights[j] -= learningRate * gradient[j];
                    }
            ...
            //check whether the # of instances % batchSize ==0, if not then edge case
                ...
                for(int j=0; j < temp_weights.length; j++){
                    ...
                    int edgeNumber = NumberInstances - i;
                    List<Instance> instances = dataSet.getInstances(i, edgeNumber);
                    double gradient[] = nn.getGradient(instances);
