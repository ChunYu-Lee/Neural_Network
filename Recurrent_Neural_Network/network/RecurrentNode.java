...
public class RecurrentNode {
    ...
    public void applyLinear(int timeStep) {
        ...
        this.postActivationValue[timeStep] = (this.preActivationValue[timeStep] + this.bias);
    }
    ...
    public void applySigmoid(int timeStep) {
        //TODO: You need to implement this for Programming Assignment 2 - Part 1
        this.postActivationValue[timeStep] = 1.0 / (1.0 + Math.exp(-(this.preActivationValue[timeStep] + this.bias)));
    }
    ...
    public void applyTanh(int timeStep) {
        //TODO: You need to implement this for Programming Assignment 2 - Part 1
        this.postActivationValue[timeStep] = Math.tanh(this.preActivationValue[timeStep] + this.bias);
    }
    ...
    public void propagateForward(int timeStep) {
        ...
        //Only apply activation function to hidden layer and output layer. RNN has set the input value to the postActivationValue in input node.
        ...
        //propagate the postActivationValue to all the outputNode. 
        for(Edge edge: outputEdges){
        ...
        //propagate the postActivationValue to all the recurrent outputNode.
        for(RecurrentEdge recurrentedge: outputRecurrentEdges){
            
            recurrentedge.outputNode.preActivationValue[timeStep + recurrentedge.timeSkip] += (recurrentedge.weight * this.postActivationValue[timeStep]); 
            } 
    }
    ...
    public void propagateBackward(int timeStep) throws NeuralNetworkException{
        ...
        //calculate the delta through activation type
        
        //if (this.nodeType == NodeType.OUTPUT || this.nodeType == NodeType.HIDDEN){ 
            if(this.activationType == ActivationType.LINEAR){
                this.delta[timeStep] *= 1.0;
            }else if(this.activationType == ActivationType.SIGMOID){
                this.delta[timeStep] *= (this.postActivationValue[timeStep] *(1.0 - this.postActivationValue[timeStep]));
            }else if(this.activationType == ActivationType.TANH){
                this.delta[timeStep] *= (1.0 - (this.postActivationValue[timeStep] * this.postActivationValue[timeStep]));
            } 
        
            this.biasDelta += this.delta[timeStep];
        
            //propagate backward through input edges.
            for (Edge edge: inputEdges){
                edge.propagateBackward(timeStep, this.delta[timeStep]);
                }

            //propagate backward through input Recurrent edges.
