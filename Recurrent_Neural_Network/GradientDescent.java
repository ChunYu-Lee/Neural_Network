...
public class GradientDescent {
    ...
    public static void main(String[] arguments) {
        ...
        try {
            ...
            //TODO: For Programming Assignment 2 - Part 3 use this and implement nesterov momentum
            ...
            for (int i = 0; i < epochs; i++) {
                ...
                if (descentType.equals("stochastic")) {
                    //TODO: Programming Assignment 2 - Part 3 you need to implement one epoch (pass through the
                    ...
                    for(int k=0; k < trainingDataSet.getNumberSequences(); k++){

                        double gradient[] = rnn.getGradient(trainingDataSet.getSequence(k));
                        double temp_weights[] = rnn.getWeights();
                        ...
                        //sum the square of weight
                        double sumSquare = 0.0;
                        for (double weight: temp_weights){
                            sumSquare += (weight*weight);
                            }

                        //calculate the L2-Norm
                        double l2 = Math.sqrt(sumSquare);
                        if ( l2 > highThreshold){
                            //grading scaling
                            for (int pos = 0; pos < temp_weights.length; pos++){
                                temp_weights[pos] *= (highThreshold/l2);
                                }
                        }else if ( l2 < lowThreshold){
                            //gradient boosting
                            for (int pos = 0; pos < temp_weights.length; pos++){
                                temp_weights[pos] *= (lowThreshold/l2);
                                }
                        	}

                    	rnn.setWeights(temp_weights);
                	}
                    ...
                    int NumberSequences = trainingDataSet.getNumberSequences();
                    double[] gradient = new double[rnn.getNumberWeights()];

                    for(int k=0; k < NumberSequences; k+= batchSize){

                        double temp_weights[] = rnn.getWeights();

                        if (k < NumberSequences - batchSize){

                            List<Sequence> sequences = trainingDataSet.getSequences(k, batchSize);
                            gradient = rnn.getGradient(sequences);

                        }else{

                            if (NumberSequences % batchSize == 0){
                                ...
                                int edgeNumber = NumberSequences - 1;
                                List<Sequence> sequences = trainingDataSet.getSequences(k, edgeNumber);
                                gradient = rnn.getGradient(sequences);
                                }
                            }
                    ...
                    //TODO: Programming Assignment 2 - Part 3 you need to implement one epoch (pass through the training
                    //sequences) for batch gradient descent
                    //TODO: Programming Assignment 2 - Part 4 - first apply nesterov momentum and then apply gradient boosting if the
                    //low threshold is > 0 and grading scaling if the high threshold is > 0

                    double gradient[] = rnn.getGradient(trainingDataSet.getSequences());
                    double temp_weights[] = rnn.getWeights();
